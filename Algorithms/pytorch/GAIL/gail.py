...
class GAIL:
    ...
    def _init_model(self):
        ...
        self.value = Value(dim_state=self.config["value"]["dim_state"],
                           ...
                           activation=resolve_activate_function(self.config["value"]["activation"])
                           )
    ...
    def learn(self, writer, i_iter):
        memory, log = self.collector.collect_samples(self.config["ppo"]["sample_batch_size"])
        ...
        writer.add_scalar('accuracy/expert', expert_r.mean().item(), i_iter)
        writer.add_scalar('accuracy/gen', gen_r.mean().item(), i_iter)
