...
class ExpertDataset(object):
    ...
    def __init__(self, expert_path=None, traj_data=None, train_fraction=0.7, batch_size=64,
        ...
        if 'state' in traj_data:
            observations = traj_data['state']
        else:
            observations = traj_data['obs']
        if 'action' in traj_data:
            actions = traj_data['action']
        else:
            actions = traj_data['acs']
        if 'ep_reward' in traj_data:
            ep_ret = traj_data['ep_reward']
        else:
            ep_ret = traj_data['ep_rets']

        if traj_limitation < 0:
            traj_limitation = len(ep_ret)

        observations = observations[:traj_limitation]
        actions = actions[:traj_limitation]
        self._num_states = observations.shape[-1]
        self._num_actions = actions.shape[-1]
        ...
        self.returns = ep_ret[:traj_limitation]
        self.avg_ret = sum(self.returns) / len(self.returns)
        self.std_ret = np.std(np.array(self.returns))
        self.verbose = verbose

        assert len(self.observations) == len(self.actions), "The number of observations and actions differ " \
    ...
    def log_info(self):
        ...
        """
        print("Total trajectories: {}".format(self.num_traj))
        print("Total transitions: {}".format(self.num_transition))
        print("Average returns: {}".format(self.avg_ret))
        print("Std for returns: {}".format(self.std_ret))
