...
class DQN:
    ...
    def choose_action(self, state):
        state = np.expand_dims(np.array(state, dtype=NFLOAT), axis=0)
    ...
    def update(self, batch):
        batch_state = np.array(batch.state, dtype=NFLOAT)
        batch_action = np.array(batch.action, dtype=np.int64)
        batch_reward = np.array(batch.reward, dtype=NFLOAT)
        batch_next_state = np.array(batch.next_state, dtype=NFLOAT)
        batch_mask = np.array(batch.mask, dtype=NFLOAT)
